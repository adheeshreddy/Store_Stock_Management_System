...
class ProformaServiceUnitTest {
    ...
    @Mock private ProformaDao proformaDao;
    @Mock private PurchaseLookupDao purchaseLookupDao;
    @Mock private PurchaseDao purchaseDao;
    @Mock private StockOutService stockOutService;
    @Mock private ProformaValidator validator; 
    ...
    void init() {
        ...
        doNothing().when(validator).validateDelete(any());
        doNothing().when(validator).validateCreateOrEdit(any());
        doNothing().when(validator).validateApprove(any());
    }
    ...
    private ProformaHeader sampleHeader(Long id) {
        ...
        ProformaDetails d1 = new ProformaDetails(id, 101L, "P1001", 10, 100.0, 18.0, 118.0, LocalDate.now().plusDays(90), "C");
        ProformaDetails d2 = new ProformaDetails(id, 102L, "P2001", 5, 50.0, 9.0, 59.0, LocalDate.now().plusDays(60), "C");
    ...
    void editProforma_nullId_throws() {
        ProformaHeader in = sampleHeader(null);
        assertThatThrownBy(() -> service.editProforma(in))
    ...
    void deleteProforma_notFound_throws() {
        ...
        assertThatThrownBy(() -> service.deleteProforma(in))
                .isInstanceOf(BadRequestException.class)
                .hasMessageContaining("Proforma not found");
